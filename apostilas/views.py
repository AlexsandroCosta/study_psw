...
def adicionar_apostilas(request):
    if request.method == 'GET':
        ...
        views_totais = ViewApostila.objects.filter(apostilas__user=request.user).count()
...
def apostila(request, id):
    ...
    views_totais = ViewApostila.objects.filter(apostilas=apostila).count()
    views_unicas = ViewApostila.objects.filter(apostilas=apostila).values('ip').distinct().count()
