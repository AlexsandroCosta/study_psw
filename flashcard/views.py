...
def deletar(request, id):
    ...
    if flashcard.user == request.user:
        ...
        messages.add_message(request, constants.ERROR, 'Flashcard não foi deletado, pois não é seu.')
...
def responder(request, id):
    flashcard = FlashcardDesafio.objects.get(id=id)
    acertou = request.GET.get('acertou')
    desafio_id = request.GET.get('desafio_id')

    if not flashcard.flashcard.user == request.user:
        raise Http404()

    flashcard.respondido = True
    flashcard.acertou = True if acertou == '1' else False
